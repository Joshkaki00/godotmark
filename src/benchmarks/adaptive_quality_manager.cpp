...
void AdaptiveQualityManager::_bind_methods() {
    ClassDB::bind_method(D_METHOD("initialize", "starting_preset"), &AdaptiveQualityManager::initialize, DEFVAL(MEDIUM));
    ClassDB::bind_method(D_METHOD("update", "current_fps", "temperature"), &AdaptiveQualityManager::update);
    ClassDB::bind_method(D_METHOD("set_verbose_logging", "enabled"), &AdaptiveQualityManager::set_verbose_logging);
    ClassDB::bind_method(D_METHOD("get_verbose_logging"), &AdaptiveQualityManager::get_verbose_logging);
    
    ClassDB::bind_method(D_METHOD("set_quality_preset", "preset"), &AdaptiveQualityManager::set_quality_preset);
    ClassDB::bind_method(D_METHOD("get_quality_preset"), &AdaptiveQualityManager::get_quality_preset);
    ClassDB::bind_method(D_METHOD("get_quality_name"), &AdaptiveQualityManager::get_quality_name);
    
    ClassDB::bind_method(D_METHOD("get_texture_resolution"), &AdaptiveQualityManager::get_texture_resolution);
    ClassDB::bind_method(D_METHOD("get_shadows_enabled"), &AdaptiveQualityManager::get_shadows_enabled);
    ClassDB::bind_method(D_METHOD("get_shadow_quality"), &AdaptiveQualityManager::get_shadow_quality);
    ClassDB::bind_method(D_METHOD("get_particle_count"), &AdaptiveQualityManager::get_particle_count);
    ClassDB::bind_method(D_METHOD("get_physics_bodies"), &AdaptiveQualityManager::get_physics_bodies);
    ClassDB::bind_method(D_METHOD("get_post_processing"), &AdaptiveQualityManager::get_post_processing);
    
    ClassDB::bind_method(D_METHOD("can_upgrade"), &AdaptiveQualityManager::can_upgrade);
    ClassDB::bind_method(D_METHOD("can_downgrade"), &AdaptiveQualityManager::can_downgrade);
    ClassDB::bind_method(D_METHOD("get_status"), &AdaptiveQualityManager::get_status);
    ClassDB::bind_method(D_METHOD("reset_hysteresis"), &AdaptiveQualityManager::reset_hysteresis);
...
void AdaptiveQualityManager::initialize(int starting_preset) {
    ...
    UtilityFunctions::print("[AdaptiveQuality] Initialized at: ", get_quality_name());
}
...
void AdaptiveQualityManager::update(float current_fps, float temperature) {
    ...
    if (temperature > TEMP_THROTTLE_THRESHOLD) {
        if (can_downgrade()) {
            if (verbose_logging) {
                UtilityFunctions::print("[AdaptiveQuality] Temperature ", String::num(temperature, 1), 
                                       "°C → Downgrading to ", presets[current_preset - 1].name.utf8().get_data());
            } else {
                UtilityFunctions::print("[AdaptiveQuality] Thermal throttle → ", presets[current_preset - 1].name.utf8().get_data());
            }
    ...
    if (current_fps < MIN_FPS) {
        ...
        if (verbose_logging && time_below_target > 0.5f) {
            UtilityFunctions::print("[AdaptiveQuality] Below target for ", String::num(time_below_target, 1), "s (FPS: ", String::num(current_fps, 1), ")");
        }
    } else if (current_fps > UPGRADE_FPS) {
        time_above_target += delta;
        time_below_target = 0.0f;
        
        if (verbose_logging && time_above_target > 1.0f && (int(time_above_target * 10) % 10 == 0)) {
            UtilityFunctions::print("[AdaptiveQuality] Above target for ", String::num(time_above_target, 1), "s (FPS: ", String::num(current_fps, 1), ")");
        }
    } else {
    ...
    if (time_below_target >= DOWNGRADE_TIME) {
        if (can_downgrade()) {
            UtilityFunctions::print("[AdaptiveQuality] FPS below ", String::num(MIN_FPS, 1), 
                                   " for ", String::num(time_below_target, 1), "s → Downgrading to ", 
        ...
        if (can_upgrade()) {
            UtilityFunctions::print("[AdaptiveQuality] FPS above ", String::num(UPGRADE_FPS, 1),
                                   " for ", String::num(time_above_target, 1), "s → Upgrading to ", 
                                   presets[current_preset + 1].name.utf8().get_data());
            set_quality_preset(current_preset + 1);
            reset_hysteresis();
        }
...
void AdaptiveQualityManager::set_quality_preset(int preset) {
    if (preset < POTATO || preset > ULTRA) {
        UtilityFunctions::push_warning("[AdaptiveQuality] Invalid preset: ", preset);
        return;
    }
...
void AdaptiveQualityManager::apply_preset_internal(QualityPreset preset) {
    ...
    UtilityFunctions::print("[AdaptiveQuality] Applied preset: ", settings.name.utf8().get_data());
    UtilityFunctions::print("  Texture Resolution: ", settings.texture_resolution);
    UtilityFunctions::print("  Shadows: ", settings.shadows_enabled ? "Enabled" : "Disabled");
    ...
    UtilityFunctions::print("  Post Processing: ", settings.post_processing ? "Enabled" : "Disabled");
}
