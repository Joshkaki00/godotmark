...
void BenchmarkOrchestrator::_bind_methods() {
    ClassDB::bind_method(D_METHOD("run_benchmark", "scene_name"), &BenchmarkOrchestrator::run_benchmark, DEFVAL("gpu_basics"));
    ClassDB::bind_method(D_METHOD("stop_benchmark"), &BenchmarkOrchestrator::stop_benchmark);
    
    ClassDB::bind_method(D_METHOD("get_platform_detector"), &BenchmarkOrchestrator::get_platform_detector);
    ClassDB::bind_method(D_METHOD("get_performance_monitor"), &BenchmarkOrchestrator::get_performance_monitor);
    ClassDB::bind_method(D_METHOD("get_quality_manager"), &BenchmarkOrchestrator::get_quality_manager);
    ClassDB::bind_method(D_METHOD("get_results_exporter"), &BenchmarkOrchestrator::get_results_exporter);
    
    ClassDB::bind_method(D_METHOD("get_is_initialized"), &BenchmarkOrchestrator::get_is_initialized);
    ClassDB::bind_method(D_METHOD("get_is_running"), &BenchmarkOrchestrator::get_is_running);
    ClassDB::bind_method(D_METHOD("get_current_scene"), &BenchmarkOrchestrator::get_current_scene);
}
...
void BenchmarkOrchestrator::run_benchmark(const String& scene_name) {
    ...
    if (is_running) {
        UtilityFunctions::push_warning("[BenchmarkOrchestrator] Benchmark already running!");
        return;
    }
    ...
    UtilityFunctions::print("[BenchmarkOrchestrator] Starting benchmark: ", scene_name);
    UtilityFunctions::print("\n");
...
void BenchmarkOrchestrator::run_benchmark_internal(const String& scene_name) {
    ...
    UtilityFunctions::print("[BenchmarkOrchestrator] Benchmark scene is now running.");
    UtilityFunctions::print("[BenchmarkOrchestrator] Performance monitoring active.");
    UtilityFunctions::print("[BenchmarkOrchestrator] Adaptive quality management active.");
    UtilityFunctions::print("\n");
}
...
void BenchmarkOrchestrator::finalize_results() {
    ...
    if (platform_detector.is_valid()) {
        results_exporter->set_platform_info(
            platform_detector->get_platform_name(),
    ...
    if (performance_monitor.is_valid()) {
        results_exporter->set_performance_data(
            performance_monitor->get_avg_fps(),
            performance_monitor->get_min_fps(),
    ...
    String filename = results_dir + current_scene_name + "_" + timestamp + ".json";
...
bool BenchmarkOrchestrator::get_is_running() const {
    return is_running;
}
